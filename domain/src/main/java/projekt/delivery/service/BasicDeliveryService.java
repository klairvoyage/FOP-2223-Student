...
public class BasicDeliveryService extends AbstractDeliveryService {
    ...
    protected List<Event> tick(long currentTick, List<ConfirmedOrder> newOrders) {
        HelperClass.helper=currentTick;
                ...
                consumer=new Consumer<Vehicle>() {
                    ...
                    public void accept(Vehicle vehicle) {
                        ...
                        for(ConfirmedOrder order:locationSpecificOrders)
                            dest.deliverOrder(vehicle,order,HelperClass.helper);              // THIS IS LIKELY WRONG!!!
                        if(vehicle.getOrders().isEmpty())
                            try{
                            vehicle.moveDirect(vehicleManager.getRegion().getNode(
                            locationSpecificOrders.get(0).getLocation()));} catch(Exception e){}
                    }
                };
